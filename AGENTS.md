...
- Phase 2 has explicit backpressure limits: `MAX_VIDEO_SECONDS`, `MAX_CONCURRENT_INGESTIONS`, `MAX_CHUNKS_PER_VIDEO`, `MAX_CHAT_HISTORY_MESSAGES`, `MAX_RETRIEVED_CHUNKS`, and `MAX_SESSIONS_PER_IP_PER_HOUR`.
- `MAX_CONCURRENT_INGESTIONS` and `MAX_SESSIONS_PER_IP_PER_HOUR` are enforced at `POST /ingest` with `429` responses. They are in-process demo safeguards, not a distributed queue or production rate limiter.
- `MAX_CHUNKS_PER_VIDEO` caps stored/embedded chunks after transcript chunking. `MAX_RETRIEVED_CHUNKS` caps Qdrant context returned to chat, while comparison routes still retrieve A and B separately before merging.
- `MAX_CHAT_HISTORY_MESSAGES` caps chat messages loaded for prompt context and frontend message reloads. `GET /config` exposes the active runtime limits so the frontend can render the same values the backend enforces.
